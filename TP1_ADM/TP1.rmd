...
print(norme_eu_c)
inertie_Chinon=sum(diag(t(x_ccr)%*%W_C%*%x_ccr))
print(inertie_Chinon)
```
...
x_cc=C-I_4%*%x_cgt
vec_norm_c=diag(diag(t(x_cc)%*%W_C%*%x_cc))
x_cc=x_cc%*%solve(sqrt(vec_norm_c))
bar=t(x_cc)%*%W_C%*%I_4 
barycentre_c=t(bar)
#print(barycentre_c)
...
## Calcul de l'inertie de chaque appelation pour trouver l'inertie inter-appelation :
inertie_Bourgueuil=sum(diag(t(x_bc)%*%W_B%*%x_bc))
print(inertie_Bourgueuil)

inertie_Chinon=sum(diag(t(x_cc)%*%W_C%*%x_cc))
print(inertie_Chinon)

inertie_Saumur=sum(diag(t(x_sc)%*%W_S%*%x_sc))
print(inertie_Saumur)
print(c(inertie_Bourgueuil, inertie_Chinon, inertie_Saumur))

inertie_Bourgueuil/inertie
