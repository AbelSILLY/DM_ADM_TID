...
```
La partition à travers les variables "Economie" permettent de capter 18% des disparités.
Variables les plus liées: salaire annuel, metre carré ancien, impot revenu
On décide de partitionner la classe Economie et 3 classes: "riches", "moyenne" et "pauvres"
...
KMV2R_DF
R2_PV2R = cbind(rep(0 , ncol(villes)))
#Puis, on calcule les R2 de toutes les variables avec la variable de classe et on met les résultats dans R2:
for (i in cbind(1:ncol(villes))) {
  R2_PV2R[i] = summary(lm(villes[,i]~as.factor(PV2R)))$r.squared}
#On peut réassigner les noms des variables aux éléments de ce vecteur
row.names(R2_PV2R) = colnames(Villes)
R2_PV2R
...
La partition à travers les variables "Risques" permettent de capter 5% des disparités.
...
KMV2N_DF
R2_PV2N = cbind(rep(0 , ncol(villes)))
#Puis, on calcule les R2 de toutes les variables avec la variable de classe et on met les résultats dans R2:
for (i in cbind(1:ncol(villes))) {
  R2_PV2N[i] = summary(lm(villes[,i]~as.factor(PV2N)))$r.squared}
#On peut réassigner les noms des variables aux éléments de ce vecteur
row.names(R2_PV2N) = colnames(Villes)
R2_PV2N
which.max(R2_PV2N)
...
La partition à travers les variables "Nature" permettent de capter 8% des disparités.
...
KMV2C_DF
R2_PV2C = cbind(rep(0 , ncol(villes)))
#Puis, on calcule les R2 de toutes les variables avec la variable de classe et on met les résultats dans R2:
for (i in cbind(1:ncol(villes))) {
  R2_PV2C[i] = summary(lm(villes[,i]~as.factor(PV2C)))$r.squared}
#On peut réassigner les noms des variables aux éléments de ce vecteur
row.names(R2_PV2C) = colnames(Villes)
R2_PV2C
which.max(R2_PV2C)
...
La partition à travers les variables "Culture" permettent de capter ~5% des disparités.
...
mIC2nat
mIC2cul
mIC2<-cbind(mIC2ec,mIC2ris,mIC2nat,mIC2cul)
...
KMV2TOT_DF
write_delim(KMV2TOT_DF,"./dfindic.txt",col_names=FALSE)
```
